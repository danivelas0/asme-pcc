...
2. **Estructura vigente de `resources/`** (verificar antes de cada uso, puede reorganizarse):
   ...
   - `resources/asme_b31/asme_b31_3/appex/` — ASME B31.3 2024: índice (`asme_b31_3_2024_index.json`) y Apéndices A, B y C:
   ...
   - `resources/asme_b31/asme_b31_3/chapters/` — cuerpo normativo del B31.3: capítulos (`chapter_01..10.json`), definiciones, figuras y `tables/` con las tablas del articulado (302.3.3-1 `Ec` incrementado, 302.3.4-1 `Ej`, 341.3.2-1, 326.1.1-1…).
     ...
     - **Hueco ya cerrado:** `table_302_3_4_1.json` perdió el cuerpo de la tabla —sus filas se colapsaron dentro de los encabezados de columna— y por eso no se podía citar de ella el factor `Ej` del para. 302.3.4(b). `completar_tabla_302_3_4.py` la **reconstruyó desde el folio impreso** que aportó el ingeniero fuera del repositorio (copyright de ASME); sus **10 filas** ya se pueden citar y el SHA-256 del PDF de origen queda en `extraction_amendments` para auditoría. El bloque `extraction_gap` desapareció al cerrarse.
